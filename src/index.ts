...
enum VhdxGuid {
  SIZE = '2FA54224-CD1B-4876-B211-5DBED83BF4B8',
  DISK_ID = 'BECA12AB-B2E6-4523-93EF-C309E000C746',
  FILE_PARAMS = 'CAA16737-FA36-4D43-B3B6-33F0AA44E76B',
  LOGICAL_SECTOR = '8141BF1D-A96F-4709-BA47-F233A8FAAB5F',
  PHYSICAL_SECTOR = 'CDA348C7-445D-4471-9CC9-E9885251C556',
  BAT_REGION = '2DC27766-F623-4200-9D64-115E9BFD4A08',
  METADATA_REGION = '8B7CA206-4790-4B9A-B8FE-575F050F886E',
}
...
const METADATA_LIST: MetadataEntry[] = [
  {
...
const BAT_REGION_ENTRY: RegionTableEntry = {
  index: 0,
  guid: VhdxGuid.BAT_REGION,
  file_offset: 2 * 1024 * 1024,
  length: 1024 * 1024,
};
const METADATA_REGION_ENTRY: RegionTableEntry = {
  index: 1,
...
export function createFile(
  ...
  createBuffer(params, (err, buffer) => {
    if (err) {
      ...
      fs.writeFile(params.path, buffer, done);
    }
  });
}
export function createBuffer(
  ...
  const metadata_map = new Map<VhdxGuid, Buffer>();
  const creator = params.creator ?? CREATOR;
  const buffer = Buffer.alloc(4 * 1024 * 1024);
  crypto.randomBytes(RANDOM_BYTES, (err, random) => {
...
function _writeHeader(buffer: Buffer, sequence_number: bigint, random: Buffer) {
...
function _writeRegionTable(buffer: Buffer) {
...
function _writeRegionTableEntry(buffer: Buffer, entry: RegionTableEntry) {
...
function _writeMetadata(buffer: Buffer, map: Map<VhdxGuid, Buffer>) {
...
function _subarray(buffer: Buffer, offset: number, size: number) {
  return buffer.subarray(offset, offset + size);
}
function _guidToBuffer(guid: string): Buffer {
  return Buffer.from(guid.replaceAll('-', ''), 'hex');
}
function _writeGuid(buffer: Buffer, guid: string, offset: number) {
...
function _zero(buffer: Buffer, start: number, length?: number): number {
  if (length) {
...
function _writeChecksum(buffer: Buffer, offset: number) {
